...
sequenceDiagram
    ...
    db_conn->>db: Opens database connection using PG and stores the connection
    db->>db_conn: Returns connection confirmation or no error

    app->>db_conn: Seeds database with music_library.sql by calling seed() method on DatabaseConnection
    db_conn->>db: Sends music_library.sql contents to database to commit
    db->>db_conn: Returns confirmation of commit or no error
    ...
    loop 
        ar->>ar: Loops through list and creates an Album object for every item
    end
